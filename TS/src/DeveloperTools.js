...
function registerSimulationInterfaces() {
  ...
  const illegalCharacters = (function () {
    ...
    if (
      csharp.UnityEngine.Application.platform ===
        csharp.UnityEngine.RuntimePlatform.WindowsEditor ||
      csharp.UnityEngine.Application.platform ===
        csharp.UnityEngine.RuntimePlatform.WindowsPlayer
    ) {
      return ['"'];
    }
    return ["*", "?", '"', "<", ">", "|"];
  ...
  puerts.registerBuildinModule("path", {
    ...
    resolve(...paths) {
      if (
        illegalCharacters.find((keyword) =>
          paths.find((path) => path.includes(keyword))
        )
      ) {
        return "";
      }
      return csharp.System.IO.Path.GetFullPath(
        csharp.System.IO.Path.Combine(...paths)
      );
    },
  });
...
try {
  if (!hasRequireInterfaces()) {
    ...
    csharp.UnityEngine.Debug.Log(
      "simulation interfaces: <color=green>registered</color>"
    );
  }
  ...
  csharp.UnityEngine.Debug.Log(
    "source-map-support: <color=green>enable</color>"
  );
} catch (e) {
  csharp.UnityEngine.Debug.LogError(
    "source-map-support module exception:" + e.message + "\n" + e.stack
  );
}
